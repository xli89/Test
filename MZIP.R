...
lMZI_wrapper = function(HFD,trt,data,try=20,family=c("poisson","negbin"),
  ...
  ### initialization ###
    ...
    if(length(inits)==1){
      if(inits==1){ 
      par <- switch(family,"negbin"=c(r0,r1,phi,b0,b1),"poisson"=c(r0,r1,b0,b1))
    
    }else if(inits==0){
      par <- switch(family,"negbin"=rep(0,5),"poisson"=rep(0,4))
     }
    }
...
fit0 <- lMZI_wrapper(HFD="HFD",trt="trt",data=sim.wide,try=20,family="poisson",method="Nelder-Mead",inits=1)

fit1 <- lMZI_wrapper(HFD="HFD",trt="trt",data=sim.wide,try=20,family="negbin",method="BFGS",inits=0)
